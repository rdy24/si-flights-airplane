            ...
            <table class="table table-striped" id="table-1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $ticket->status }}</td>
                  <td>
                    <a href="{{ route('dashboard.restore.ticket', $ticket->id) }}"
                      class="btn btn-success btn-restore"><i class="fa fa-trash-restore" aria-hidden="true"></i></a>
                    <a href="{{ route('dashboard.delete.ticket', $ticket->id) }}" class="btn btn-danger btn-delete"><i
...
<script>
  $('.btn-restore').on('click', function (e) {
    ...
    swal({
      title: 'Are you sure?',
      text: 'Data akan kembali ke daftar tiket',
      icon: 'warning',
      buttons: ["Cancel", "Yes!"],
    }).then(function(value) {
      if (value) {

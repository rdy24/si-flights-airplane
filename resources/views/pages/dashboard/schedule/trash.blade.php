...
@section('title')
Trashed Jadwal | {{ config('app.name') }}
@endsection
...
<div class="section-header">
  <h1>Trashed Data Jadwal</h1>
  <div class="section-header-breadcrumb">
    <div class="breadcrumb-item active"><a href="{{ route('dashboard.') }}">Dashboard</a></div>
    <div class="breadcrumb-item">Trashed Data Jadwal</div>
  </div>
</div>
            ...
            <table class="table table-striped" id="table-1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $schedule->route->airport_origin->name ?? '' }} - {{
                    $schedule->route->airport_destination->name ?? '' }} ({{ $schedule->route->kode_rute
                    ?? '' }})</td>
                  <td>{{ $schedule->waktu_berangkat }}</td>
                  <td>{{ $schedule->waktu_tiba }}</td>
                  <td>{{ $schedule->status }}</td>
                  <td>
                    <a href="{{ route('dashboard.restore.schedule', $schedule->id) }}" class="btn btn-success"
                      onclick="return confirm('apakah anda yakin mengembalikan data ini?')"><i
                        class="fa fa-trash-restore" aria-hidden="true"></i></a>
                    <a href="{{ route('dashboard.delete.schedule', $schedule->id) }}" class="btn btn-danger"

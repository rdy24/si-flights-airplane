...
@section('title')
Edit Data Jadwal {{ $schedule->plane->nama }}
@endsection
          ...
          <form action="{{ route('dashboard.schedule.update', $schedule->id) }}" method="POST"
            ...
            <div class="form-group">
              ...
              <select name="plane_id" required class="form-control select2">
                ...
                <option value="{{ $plane->id }}" {{ old('plane_id', $schedule->plane_id)==$plane->id ? 'selected' : ''
                  }}>
                  {{ $plane->nama }}
                </option>
                @endforeach
            ...
            <div class="form-group">
              ...
              <select name="route_id" required class="form-control select2">
                ...
                <option value="{{ $route->id }}" {{ old('route_id', $schedule->route_id)==$route->id ? 'selected' : ''
                  ...
                  {{ $route->airportOrigin->name ?? '' }} - {{ $route->airportDestination->name ?? '' }} ({{
                  $route->kode_rute ?? '' }})
                </option>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control datetimepicker" id="waktu_berangkat" name="waktu_berangkat"
                required value="{{ old('waktu_berangkat', $schedule->waktu_berangkat) }}">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control datetimepicker" id="waktu_tiba" name="waktu_tiba" required
                value="{{ old('waktu_tiba', $schedule->waktu_tiba) }}">
              @error('waktu_tiba')
              <p class="text-danger">
                {{ $message }}
              </p>
            ...
            <div class="form-group">
              ...
              <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="status" value="Menunggu Jadwal" {{
                  old('status', $schedule->status)=='Menunggu Jadwal' ? 'checked' : '' }}>
              ...
              <div class="form-check">
                <input class="form-check-input" type="radio" name="status" id="status1" value="Sedang Terbang" {{
                  old('status', $schedule->status)=='Sedang Terbang' ? 'checked' : '' }}>
